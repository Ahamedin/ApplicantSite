        ...
        <div className="lg:hidden flex items-center">
          ...
          {isOpen && (
            <ul className="absolute top-16 right-4 menu menu-lg p-4 shadow bg-base-200 rounded-xl w-60 space-y-2 z-[9999]">
              ...
              {user ? (
                <li><a href="http://localhost:3000/auth/logout" onClick={() => setIsOpen(false)}>Logout</a></li>
              ) : (
                <li><a href="http://localhost:3000/auth/google" onClick={() => setIsOpen(false)}>Login</a></li>
              )}
            </ul>
          )}
        </div>
...
<div className="hidden lg:flex items-center gap-3 justify-end flex-1">
  ...
  {user ? (
    <div className="flex items-center gap-2">
      ...
      <a href="http://localhost:3000/auth/logout" className="btn btn-sm">Logout</a>
    </div>
  ) : (
    <a href="http://localhost:3000/auth/google" className="btn btn-sm btn-primary">
      Login
    </a>
  )}
</div>
