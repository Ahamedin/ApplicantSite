...
function CartPage() {
  const { getBlock, savedblocks, loading, error } = useBlockStore()
          ...
          {savedblocks.length > 0 ? (
            ...
            ">
              {savedblocks.map((item) => (
                <CartCard key={item.id} savedblocks={item} />
              ))}
            </div>
          ) : (
<div className="text-center text-gray-500 text-lg font-serif py-20 flex flex-col items-center justify-center">
  <p className="mb-4">No items found in your cart</p>
...
<svg
  ...
  className="text-based-content"    // dynamic theme color
>
  <path
    fillRule="evenodd"
    clipRule="evenodd"
